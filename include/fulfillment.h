...
__device__ inline int fulfillment_greedy_cost(
    ...
    int nn_num_layers,
    const double* inventory_row,  // [n_nodes_plus1] for this product
    const double* capacity,       // [n_nodes_plus1]
    int quantity,
    const int* node_near_to_far,   // [n_nodes_plus1] sorted node indices
    int n_nodes_plus1,
    double* scratch_a,             // scratch buffer for NN: max(layer_sizes)
    double* scratch_b,              // scratch buffer for NN: max(layer_sizes)
    double* nn_output               // scratch buffer for NN output: layer_sizes[last]
) {
    ...
    for (int rank = 0; rank < n_nodes_plus1; rank++) {
        ...
        double inv = inventory_row[node] + round(1e-7 * nn_output[rank] / norm);
        double cap = capacity[node];
        if (cap >= (double)quantity && inv >= (double)quantity) {
            return node;
        }
    }
